...
-- 日期: 2025-12-28
-- 作者: 小阳
-- 描述: 添加动态提示词配置表，实现提示词的数据库动态管理

USE `deep_talent`;

-- ----------------------------
-- 4. 提示词配置表 (dt_prompt_config)
-- ----------------------------
DROP TABLE IF EXISTS `dt_prompt_config`;
CREATE TABLE `dt_prompt_config` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `prompt_key` varchar(64) NOT NULL COMMENT '提示词Key (唯一标识)',
  `language` varchar(16) NOT NULL DEFAULT 'zh-CN' COMMENT '语言代码 (如 zh-CN, en-US)',
  `content` longtext NOT NULL COMMENT '提示词内容模板',
  `version` int(11) NOT NULL DEFAULT 1 COMMENT '版本号',
  `is_active` tinyint(1) NOT NULL DEFAULT 1 COMMENT '是否为当前生效版本 (1:是, 0:否)',
  `description` varchar(255) DEFAULT NULL COMMENT '描述说明',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_key_lang_active` (`prompt_key`, `language`, `is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='深度天赋挖掘-提示词配置表';
...
INSERT INTO `dt_prompt_config` (`prompt_key`, `language`, `content`, `version`, `is_active`, `description`) 
VALUES ('EVALUATOR_SYSTEM_PROMPT', 'zh-CN', '你是“深度天赋挖掘机”的质检与信息抽取助手。你的目标是：评估用户回答的信息密度，并抽取可用于画像与报告的关键信息。
...
- 只在你有把握时抽取（confidence 低就不要填）。', 1, 1, '评估与信息抽取助手提示词');

-- 2. Interviewer System Prompt
INSERT INTO `dt_prompt_config` (`prompt_key`, `language`, `content`, `version`, `is_active`, `description`) 
VALUES ('INTERVIEWER_SYSTEM_PROMPT', 'zh-CN', '你是一位温和、专业、尊重边界的访谈主持人。你的目标是通过阶段化访谈，帮助用户把“隐性优势/天赋”具象化。
...
- shadow：反复困扰的模式、触发条件、代价/损失、想改变的方向。', 1, 1, '访谈主持人提示词');

-- 3. Writer System Prompt
INSERT INTO `dt_prompt_config` (`prompt_key`, `language`, `content`, `version`, `is_active`, `description`) 
VALUES ('WRITER_SYSTEM_PROMPT', 'zh-CN', '你是一位专业的天赋画像分析师。请基于用户访谈内容与画像信息，生成一份“天赋画像报告”。
